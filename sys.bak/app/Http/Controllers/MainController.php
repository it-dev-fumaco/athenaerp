...
class MainController extends Controller
{
    ...
    public function get_issued_qty($item_code, $warehouse){
        ...
        $total_issued  += DB::table('tabPacking Slip as ps')
            ...
            ->where('ps.docstatus', 0)->where('psi.status', 'Issued')
    ...
    public function view_material_issue(Request $request){
        ...
        $q = DB::table('tabStock Entry as ste')
            ...
            ->select('sted.status', 'sted.validate_item_code', 'ste.sales_order_no', 'sted.parent', 'sted.name', 'sted.t_warehouse', 'sted.s_warehouse', 'sted.item_code', 'sted.description', 'sted.uom', 'sted.qty', 'sted.owner')
    ...
    public function view_material_transfer_for_manufacture(Request $request){
        ...
        $q = DB::table('tabStock Entry as ste')
            ...
            ->whereIn('s_warehouse', $allowed_warehouses)
            ->select('sted.status', 'sted.validate_item_code', 'ste.sales_order_no', 'sted.parent', 'sted.name', 'sted.t_warehouse', 'sted.s_warehouse', 'sted.item_code', 'sted.description', 'sted.uom', 'sted.qty', 'ste.owner', 'ste.material_request', 'ste.production_order')
            ->orderByRaw("FIELD(sted.status, 'For Checking', 'Issued') ASC")
            ->get();
    ...
    public function view_material_transfer(Request $request){
        ...
        $q = DB::table('tabStock Entry as ste')
            ...
            ->whereIn('s_warehouse', $allowed_warehouses)
            ->select('sted.status', 'sted.validate_item_code', 'ste.sales_order_no', 'sted.parent', 'sted.name', 'sted.t_warehouse', 'sted.s_warehouse', 'sted.item_code', 'sted.description', 'sted.uom', 'sted.qty', 'sted.owner', 'ste.material_request')
            ->orderByRaw("FIELD(sted.status, 'For Checking', 'Issued') ASC")
            ->get();
    ...
    public function view_picking_slip(Request $request){
        ...
        $q = DB::table('tabPacking Slip as ps')
                ...
                ->select('ps.sales_order', 'psi.name AS id', 'psi.status', 'ps.name', 'ps.delivery_note', 'psi.item_code', 'psi.description', DB::raw('SUM(dri.qty) as qty'), 'psi.stock_uom', 'dri.warehouse', 'psi.owner', 'dr.customer')
                ->groupBy('ps.sales_order', 'psi.name', 'psi.status', 'ps.name', 'ps.delivery_note', 'psi.item_code', 'psi.description', 'psi.stock_uom', 'dri.warehouse', 'psi.owner', 'dr.customer')
    ...
    public function get_stock_ledger($item_code, Request $request){
        ...
        foreach($logs as $row){
            ...
            if($row->voucher_type == 'Delivery Note'){
                ...
                $ref_no = DB::table('tabPurchase Receipt Item')->where('parent', $row->voucher_no)
                    ->where('item_code', $item_code)->where('docstatus', 1)->pluck('purchase_order');
