	...
	public function readExcelFile(Request $request)
	{
		...
		try {
			if ($request->hasFile('selected-file')) {
				...
				$fileName = pathinfo($attachedFile->getClientOriginalName(), PATHINFO_FILENAME);
				$fileExt = pathinfo($attachedFile->getClientOriginalName(), PATHINFO_EXTENSION);
				...
				$project = isset($fileContents['project']) && $fileContents['project'] ? trim(str_replace('/', '-', $fileContents['project'])) : '-';
				...
				$attachedFile->move(public_path('storage/brochures/' . strtoupper($project)), $newFilename . '.' . $fileExt);

				DB::commit();

				return ApiResponse::success('/preview/' . strtoupper($project) . '/' . $newFilename . '.' . $fileExt);
			}
		} catch (Exception $e) {
			DB::rollback();
