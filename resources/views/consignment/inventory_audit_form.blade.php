                                ...
                                <table class="table" style="font-size: 8pt;" id="items-table">
                                    ...
                                    <tbody>
                                        @forelse ($itemClassification as $class => $items)
                                            ...
                                            @php
                                                $id = $row->item_code;
                                                $img = $itemImages->has($row->item_code) ? "/img/" . $itemImages[$row->item_code][0]->image_path : "/icon/no-img.png";
                                                $imgWebp = $itemImages->has($row->item_code) ? "/img/" . explode('.',$itemImages[$row->item_code][0]->image_path)[0].'.webp' : "/icon/no_img.webp";
                                                $consignedQty = array_key_exists($row->item_code, $consignedStocks) ? ($consignedStocks[$row->item_code] * 1) : 0;

                                                $imgCount = $itemImages->has($row->item_code) ? $itemImages[$row->item_code]->count() : 0;
            ...
            <div class="modal-body">
                ...
                <div class="text-center mb-2" style="font-size: 9pt;">
                    ...
                    <span class="d-block font-weight-bold mt-3">
                        {{ session()->get('transaction_date') ? \Carbon\Carbon::parse(session()->get('transaction_date'))->format('F d, Y') : now()->format('F d, Y') }}
                    </span>
...
<script>
    $(function () {
        ...
        @php
            ...
            $startDate = isset($explodeDuration[0]) ? $explodeDuration[0] : now()->format('Y-M-d');
            $endDate = isset($explodeDuration[1]) ? $explodeDuration[1] : now()->format('Y-M-d');
        @endphp
