...
<table class="table table-bordered table-hover m-0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center p-1 align-middle reorder-qty" style="font-size:12px">{{ number_format($row['warehouse_reorder_qty'] * 1) }}</td>
            <td class="text-center p-1 align-middle"><strong>{{ number_format($row['warehouse_reorder_level'] * 1) }}</strong></td>
            <td class="text-center p-1 align-middle">
                <span class="badge badge-{{ ($row['actual_qty'] > $row['warehouse_reorder_level']) ? 'success' : 'danger' }}" style="font-size: 11pt;">{{ number_format($row['actual_qty'] * 1) }}</span>
            </td>
            <td class="text-center p-1 align-middle">
                @if(!$row['existing_mr'])
