							...
							<table class="table table-hover">
								...
								<tbody>
									<tr ng-repeat="x in mi_filtered = (mi | filter:searchText | filter: fltr)">
										...
										<td class="text-center" ng-if="x.status === 'Received'"><span class="badge badge-success">@{{ x.status }}</span></td>
										<td class="text-center" ng-if="x.status === 'To Receive'"><span class="badge badge-warning">@{{ x.status }}</span></td>
										<td class="text-center"><img src="dist/img/icon.png" class="img-circle rece1ive-item checkout" data-id="@{{ x.name }}"></td>
									</tr>
								</tbody>
							</table>
						</div>
