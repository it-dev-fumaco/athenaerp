<div class="row p-1 bg-white">
            ...
            <div class="row">
                @php
                    ...
                    $img4Webp = data_get($itemImages, 3)
                        ...
                        : '/icon/no_img.webp';
                    $img4Alt = data_get($itemImages, 3)
                        ? Illuminate\Support\Str::slug(explode('.', $img4)[0], '-')
                        : null;
                @endphp
                <div class="col-md-3">
                    <div class="row">
                        ...
                        <div class="col-4 mt-2">
                            <a href="{{ Storage::disk('upcloud')->url($img2) }}" data-toggle="lightbox"
                                ...
                                @php
                                    $webpPath = 'img/' . explode('.', $img2Name)[0] . '.webp';
                                @endphp
                        ...
                        <div class="col-4 mt-2">
                            <a href="{{ Storage::disk('upcloud')->url($img3) }}" data-toggle="lightbox"
                                ...
                                @php
                                    $webpPath = 'img/' . explode('.', $img3Name)[0] . '.webp';
                                @endphp
                        ...
                        <div class="col-md-12 text-center pt-3">
                            <button class="btn btn-primary btn-sm upload-item-image"
                                data-item-code="{{ $itemDetails->name }}">Upload Image(s)</button>
                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                    ...
                    <dl>
                        <dt class="responsive-item-code" style="font-size: 14pt;"><span
                                id="selected-item-code">{{ $itemDetails->name }}</span> {{ $itemDetails->brand }}
                        </dt>
                        <dd class="responsive-description" style="font-size: 11pt;" class="text-justify mb-2">
                            {!! $itemDetails->description !!}</dd>
                    </dl>
                    <div class="d-block d-lg-none">
                        <p class="mt-2 mb-2 text-center">
                            @if (!in_array($userGroup, ['Warehouse Personnel']) && $defaultPrice > 0)
                                <span class="d-block font-weight-bold"
                                    style="font-size: 17pt;">{{ '₱ ' . number_format($defaultPrice, 2, '.', ',') }}</span>
                            ...
                            @if (in_array($userGroup, ['Manager', 'Director']) && $minimumSellingPrice > 0)
                                <span class="d-block font-weight-bold"
                                    style="font-size: 15pt;">{{ '₱ ' . number_format($minimumSellingPrice, 2, '.', ',') }}</span>
                    ...
                    <div class="card-header border-bottom-0 p-1">
                        ...
                        @if (in_array($userGroup, ['Warehouse Personnel', 'Inventory Manager']))
                            <button class="btn btn-primary p-1 float-right" id="warehouse-location-btn"
                                data-item-code="{{ $itemDetails->name }}" style="font-size: 12px;">Update Warehouse
                                Location</button>
                        @endif
                    </div>
                        ...
                        <div class="box-header with-border">
                            <div class="box-body table-responsive">
                                <table class="table table-striped table-bordered table-hover"
                                    ...
                                    <thead>
                                        <tr>
                                            <th scope="col" rowspan=2 class="font-responsive text-center p-1">
                                                Warehouse</th>
                                            <th scope="col" colspan=3 class="font-responsive text-center p-1">
                                                Quantity</th>
                                        </tr>
                                        <tr>
                                            <th scope="col" class="font-responsive text-center p-1 text-muted">
                                                Reserved</th>
                                    ...
                                    @forelse ($siteWarehouses as $stock)
                                        <tr>
                                            ...
                                            <td class="text-center p-1 font-responsive">
                                                <small
                                                    class="text-muted">{{ number_format((float) $stock['reserved_qty'], 2, '.', '') . ' ' . $stock['stock_uom'] }}</small>
                                            </td>
                                            <td class="text-center p-1 font-responsive">
                                                {{ number_format((float) $stock['actual_qty'], 2, '.', '') . ' ' . $stock['stock_uom'] }}
                                            </td>
                                            <td class="text-center p-1">
                                                <span
                                                    class="badge badge-{{ $stock['available_qty'] > 0 ? 'success' : 'secondary' }} font-responsive"
                                                    style="font-size: 11pt;">{{ number_format((float) $stock['available_qty'], 2, '.', '') . ' ' . $stock['stock_uom'] }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                ...
                                @if (count($consignmentWarehouses) > 0)
                                    <div class="text-center">
                                        <a href="#" class="btn btn-primary uppercase p-1" data-toggle="modal"
                                            data-target="#vcww{{ $itemDetails->name }}" style="font-size: 12px;">View
                                            Consignment Warehouse</a>
                                    </div>

                                    <div class="modal fade" id="vcww{{ $itemDetails->name }}" tabindex="-1"
                                        role="dialog">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title">{{ $itemDetails->name }} - Consignment
                                                        Warehouse(s) </h4>
                                                    <button type="button" class="close"
                                                        onclick="close_modal('#vcww{{ $itemDetails->name }}')"
                                                        ...
                                                        @forelse($consignmentWarehouses as $con)
                                                            <tr>
                                                                ...
                                                                <td class="text-center"><span
                                                                        class="badge badge-{{ $con['available_qty'] > 0 ? 'success' : 'secondary' }}"
                                                                        style="font-size: 15px; margin: 0 auto;">{{ $con['actual_qty'] * 1 . ' ' . $con['stock_uom'] }}</span>
                                                                </td>
                                                            </tr>
                                                        @empty
                                                            <tr>
                                                                <td class="text-center font-italic" colspan="3">NO
                                                                    WAREHOUSE ASSIGNED</td>
                                                            </tr>
                                                        @endforelse
                                                    </table>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-default"
                                                        onclick="close_modal('#vcww{{ $itemDetails->name }}')">Close</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="box box-solid">
            ...
            <div class="divs tab-content">
                @for ($i = 0; $i < count($variants); $i++)
                    <div id="mob-variant-page-{{ $i + 1 }}"
                        class="mob-tab tab-pane {{ $i == 0 ? 'active' : null }}">
                        @php
                            $firstItemName = \Illuminate\Support\Str::limit($itemDetails->item_name, 30, $end = '...');
                        @endphp
                        <button class="btn w-100 text-left mb-3" type="button" data-toggle="collapse"
                            data-target="#variant-data-{{ $itemDetails->name }}" aria-expanded="false"
                            aria-controls="multiCollapseExample2"
                            style="font-size: 9pt;border-bottom: 2px solid #28A745; color: #28A745;"><b>{{ $itemDetails->name }}</b>
                            - {{ $firstItemName }} <i class="fa fa-chevron-down float-right"></i></button>
                        ...
                        <div class="collapse multi-collapse show" id="variant-data-{{ $itemDetails->name }}">
                            <table class="table" style="font-size: 9pt;">
                                @foreach ($attributeNames as $attributeName)
                                    <tr>
                                        @php
                                            $attributeValue = collect($attributes)
                                                ->where('parent', $itemDetails->name)
                        ...
                        @foreach ($variants[$i] as $variant)
                            ...
                            @endphp
                            <button class="btn w-100 text-left mb-3" type="button" data-toggle="collapse"
                                data-target="#variant-data-{{ $variant->name }}" aria-expanded="false"
                                aria-controls="multiCollapseExample2"
                                style="font-size: 9pt; border-bottom: 1px solid #C4C4C4"><b>{{ $variant->name }}</b> -
                                {{ $itemName }} <i class="fa fa-chevron-down float-right"></i></button>
                ...
                @endfor
                <button class="btn float-left" id="btn-prev" style="font-size: 9pt; color: #007BFF"><i
                        class="fa fa-chevron-left"></i> Previous</button>
                <button class="btn float-right" id="btn-next" style="font-size: 9pt; color: #007BFF">Next <i
                                ...
                                @foreach ($attributeNames as $attributeName)
                                    @php
                                        $attributeValue = collect($attributes)
                                            ...
                                            ->first();
                                    @endphp
                                    <td class="text-center table-highlight pb-3 pt-3">
                                        {{ $attributeValue ? $attributeValue : 'n/a' }}</td>
                                @endforeach
                                <td class="text-center table-highlight pb-3 pt-3">
                                    {{ $defaultPrice > 0 ? '₱ ' . number_format($defaultPrice, 2, '.', ',') : 'n/a' }}
                                </td>
                            </tr>
                            @foreach ($variants[$i] as $variant)
                                @if ($itemDetails->name == $variant->name)
                                ...
                                <tr style="font-size: 9pt;">
                                    ...
                                    <td class="text-center">
                                        {{ $price > 0 ? '₱ ' . number_format($price, 2, '.', ',') : 'n/a' }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
            ...
            @if (count($variants) > 1)
                <ul class="nav nav-tabs variant-tabs mb-3" role="tablist">
                    @foreach ($variants as $i => $item)
                        <li class="nav-item">
                            <a class="nav-link {{ $loop->first ? 'active' : null }}" data-toggle="tab"
                                href="#variant-page-{{ $i + 1 }}">{{ $i + 1 }}</a>
                        </li>
                    @endforeach
                </ul>
            @endif
    ...
    <div class="col-md-12 item-alternatives-section min-width-0">
        ...
        <style>
            ...
            .item-alternative-description p {
                                ...
                                <div class="col-12">
                                    <div class="d-flex flex-row">
                                        <div class="pt-2 pb-2 pr-1 pl-1">
                                            @php
                                                $img = $a['item_alternative_image']
                                                    ? '/img/' . explode('.', $a['item_alternative_image'])[0] . '.jpg'
                                                    : '/icon/no_img.jpg';
                                                $imgWebp = $a['item_alternative_image']
                                                    ? '/img/' . explode('.', $a['item_alternative_image'])[0] . '.webp'
                                                    : '/icon/no_img.webp';
                                            @endphp
                                            <a href="{{ Storage::disk('upcloud')->url($img) }}" data-toggle="lightbox"
                                                data-gallery="{{ $a['item_code'] }}"
                                                data-title="{{ $a['item_code'] }}">
                                                @if (!Storage::disk('upcloud')->exists('/img/' . explode('.', $a['item_alternative_image'])[0] . '.webp'))
                                                    <img src="{{ Storage::disk('upcloud')->url($img) }}" class="rounded"
                                                        width="80" height="80">
                                                @elseif(!Storage::disk('upcloud')->exists('/img/' . $a['item_alternative_image']))
                                                    <img src="{{ Storage::disk('upcloud')->url($img) }}" class="rounded"
                                                        width="80" height="80">
                                                @else
                                                    <picture>
                                                        <source srcset="{{ Storage::disk('upcloud')->url($imgWebp) }}"
                                                            type="image/webp" class="rounded" width="80"
                                                            height="80">
                                                        <source srcset="{{ Storage::disk('upcloud')->url($img) }}"
                                                            type="image/jpeg" class="rounded" width="80"
                                                            height="80">
                                                        <img src="{{ Storage::disk('upcloud')->url($img) }}" class="rounded"
                                                            width="80" height="80">
                                                    </picture>
                                        ...
                                        <a href="#" class="view-item-details text-dark"
                                            data-item-code="{{ $a['item_code'] }}"
                                            data-item-classification="{{ $itemDetails->item_classification }}">
                                            <div class="p-1 text-justify">
                                                <span
                                                    class="font-weight-bold font-responsive">{{ $a['item_code'] }}</span>
                                                <small class="font-italic font-responsive item-alternative-description"
                                                    style="font-size: 9pt;">{!! $a['description'] !!}</small>
                                                <br>
                                                <span
                                                    class="badge badge-{{ $a['actual_stocks'] > 0 ? 'success' : 'secondary' }} font-responsive">{{ $a['actual_stocks'] > 0 ? 'In Stock' : 'Unavailable' }}</span>
                                            </div>
                                        </a>
                                    </div>
                                </div>
