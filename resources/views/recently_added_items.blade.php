    ...
    <li class="item">
        <div class="product-img">
            @php
                ...
                $imgWebp = ($item['image']) ? "/img/" . explode('.', $item['image'])[0] : "/icon/no_img.webp";
            @endphp
            <a href="{{ asset('storage/') }}{{ $img }}" data-toggle="lightbox" data-gallery="{{ $item['item_code'] }}" data-title="{{ $item['item_code'] }}">
                ...
                @elseif(!Storage::disk('public')->exists('/img/'.$item['image']))
                    <img src="{{ asset('storage/').$imgWebp }}" class="img-size-50">
                @else
                    <picture>
                        <source srcset="{{ asset('storage'.$imgWebp) }}" type="image/webp" class="img-size-50">
